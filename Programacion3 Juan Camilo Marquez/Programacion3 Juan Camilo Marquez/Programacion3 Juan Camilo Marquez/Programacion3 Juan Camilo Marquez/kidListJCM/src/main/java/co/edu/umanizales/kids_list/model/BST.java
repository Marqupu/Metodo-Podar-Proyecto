...
public class BST {
    ...
    // Método prune para nodos con hijos
    public List<Kid> prune() {
        List<Kid> prunedKids = new ArrayList<>();
        pruneRec(root, prunedKids);
        return prunedKids;
    }

    private void pruneRec(NodeBST node, List<Kid> prunedKids) {
        if (node != null) {
            if (node.getLeft() != null || node.getRight() != null) {
                prunedKids.add(node.getKid());
            }
            pruneRec(node.getLeft(), prunedKids);
            pruneRec(node.getRight(), prunedKids);
        }
    }
}